...
void Serialize(const SparseMatrix<T, whatever, IND>& mat, std::string filename) {
    std::vector<Trip> res;
    int sz = mat.nonZeros();
...
template <typename T, int whatever, typename IND>
void Deserialize(SparseMatrix<T, whatever, IND>& m, std::string filename) {
...
bool save_problem(const Problem<Scalar>& problem)
{
    ...
    Serialize(problem.A, filename1);
    // std::cout << problem.A << "\n" << std::endl;
    // printf("SERIALIZING\n");
    // Eigen::SparseMatrix<Scalar> A_;
    // Deserialize(A_, filename1);
    // std::cout << A_ << "\n" << std::endl;
