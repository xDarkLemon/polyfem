...
inline void EigenSparseToCuSparseTranspose(const Eigen::SparseMatrix<double> &mat, int *&row, int *&col, double *&val)
{
    ...
    // cudaMalloc((void**)&row, num_outer*sizeof(int));
    // cudaMalloc((void**)&col, num_non0*sizeof(int));
    // cudaMalloc((void**)&val, num_non0*sizeof(double));
    ...
    // int &hessian_nnz, double *&hessian_val, int *&hessian_row, int *&hessian_col)
{
    ...
    CuSparseMatrixAdd(
        ...
        /*hessian_nnz,*/ hessian.nonZeros(), hessian_val, hessian_row, hessian_col,
...
inline void PartialHessianSum(
    const std::vector<Eigen::SparseMatrix<double>> &all_tmp,
    Eigen::SparseMatrix<double> &hessian)
{
    igl::Timer timerg;
    ...
    // int cnt = 0;
    size_t cnt = all_tmp.size();
    for (int i=0; i<cnt; i++)
    {
        Eigen::SparseMatrix<double> tmp = all_tmp[i];
        // printf("AFTER computing tmp at round %d\n", cnt);
        printf("EXTRACTING tmp at round %ld\n", cnt);
        printf("\ntmp eigen matrix:");
        printCSRMatrix(tmp.valuePtr(), tmp.outerIndexPtr(), tmp.innerIndexPtr(), tmp.cols(), tmp.nonZeros());
        // printf("\nhessian eigen matrix:");
        // printCSRMatrix(hessian.valuePtr(), hessian.outerIndexPtr(), hessian.innerIndexPtr(), hessian.cols(), hessian.nonZeros());
        ...
        if (!hessian.nonZeros()>0)
        {
            ...
            // hessian_col = ALLOCATE_GPU<int>(hessian_col, tmp.nonZeros()*sizeof(int));
            // hessian_val = ALLOCATE_GPU<double>(hessian_val, tmp.nonZeros()*sizeof(double));            
            // hessian_row = ALLOCATE_GPU<int>(hessian_row, (tmp.cols()+1)*sizeof(int));
            // hessian_col = ALLOCATE_GPU<int>(hessian_col, tmp.nonZeros()*sizeof(int));
            // hessian_val = ALLOCATE_GPU<double>(hessian_val, tmp.nonZeros()*sizeof(double));
            ...
            // EigenSparseToCuSparseTranspose(tmp, hessian_row, hessian_col, hessian_val);
        ...
        else
        {
            ...
            timerg.stop();
            printf("[2022-11-24 10:44:06.870] [polyfem] [debug] [timing] CUSPARSE ADD TOTAL %lfs\n", timerg.getElapsedTime());
        }
        cnt++;
    }
